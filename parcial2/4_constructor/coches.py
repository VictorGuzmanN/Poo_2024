...
"""
METODO CONSTRUCTOR.- Este metodo especial dentro de una clase y se utiliza para dar un valor a los atributos
del objeto al crearlo, es el primer metodo que se ejecuta al crear el objeto y se manda llamar automaticamente al 
crearlo, es decir, este metodo puede recibir parametros al momento de crear el objeto.

Cuando se crea un metodo constuctor se utiliza la funcion init() para que se llame automaticamente cada vez que se
utiliza la clase para crear un nuevo objeto

El self es un parametro, es una referencia a la instancia actual de la clase y se utiliza para cceder a variables 
que pertenecen a la clase

No es mnecesaerio que tenga nombre self puedes llamarlo como quieras, pero tiene que ser el primer parametro de cuaquier
funcion de la clase. Es decir por regla se utiliza la palabra self pero puede ser llamado con otros nombres por ejemplo
valor, abc, parametro, etc.
"""
